/**
 * Fungsi utama untuk mengambil data produksi cabang
 * Menampilkan data produksi dalam bentuk tabel dengan filter
 */
function getDataProduksiCabang() {
    // Sembunyikan kolom wilayah
    jQuery(".wilayah-column").hide();

    // Ambil nilai filter dari form
    const filters = getFilterValues();

    // Kirim request AJAX untuk mengambil data
    fetchProduksiData(filters);

    toggleInvoiceColumn();
}

/**
 * Mengambil nilai-nilai filter dari form
...
/**
 * Mengambil tanggal mulai dari range picker
...
/**
 * Mengambil tanggal akhir dari range picker
 * @param {string} dateRange - Nilai dari date range picker
...
/**
 * Mengirim request AJAX untuk mengambil data produksi
 * @param {Object} filters - Object berisi filter yang akan dikirim
 */
function fetchProduksiData(filters) {
    jQuery.ajax({
        type: 'POST',
        ...
        success: function (data) {
            ...
            const htmlContent = generateProduksiHTML(data);
...
/**
 * Generate HTML untuk tabel produksi
 * @param {Object} data - Data dari server
 * @returns {string} HTML string untuk tabel
 */
function generateProduksiHTML(data) {
    if (!data.data || data.data.length === 0 || data.data.extra === 0) {
        return generateEmptyTableHTML();
    }
    ...
    jQuery.each(data.data, function (i, val) {
        ...
        if (shouldDisplayData(val, arsipValue, sisaKirimSJ)) {
            htmlRows += generateRowHTML(val, i, nomor, nota, sisaKirimSJ, data, now);
    ...
    htmlRows += generateFooterHTML(totalQtyProduksi);

    return htmlRows;
}

/**
 * Cek apakah data harus ditampilkan berdasarkan kondisi
 * @param {Object} val - Data item
 * @param {string} arsipValue - Nilai arsip
 * @param {number} sisaKirimSJ - Sisa kirim surat jalan
 * @returns {boolean}
 */
function shouldDisplayData(val, arsipValue, sisaKirimSJ) {
    if (arsipValue === 'empty') {
        // Kondisi untuk data non-arsip
        ...
        // Kondisi untuk data arsip
        if (val.extra === null || val.extra === 1) {
            return val.total_terima_pabrik >= val.penjualan_qty;
        }
    }
    return false;
}
...
/**
 * Generate HTML untuk satu baris data produksi
 * @param {Object} val - Data item
 ...
 */
function generateRowHTML(val, index, nomor, nota, sisaKirimSJ, allData, now) {
    let html = '';

    // Tentukan warna background berdasarkan status
    const backgroundColor = getStatusBackgroundColor(val.status_produksi);
    const warnaTelatInfo = getWarnaTelatInfo(val.penjualan_tanggal_kirim);

    // Jika ini adalah nota baru, buat header row
    if (nota !== val.penjualan_id) {
        html += generateHeaderRow(val, nomor, warnaTelatInfo);
    } else {
        html += generateContinuationRow(val, nomor, warnaTelatInfo);
    }

    // Tambahkan kolom-kolom data
    html += generateDataColumns(val, index, backgroundColor, warnaTelatInfo, sisaKirimSJ, allData, now);
    html += '</tr>';
...
/**
 * Mendapatkan warna background berdasarkan status produksi
...
/**
 * Generate HTML untuk header row (nota baru)
 * @param {Object} val - Data item
 * @param {number} nomor - Nomor urut
 * @param {Object} warnaTelatInfo - Informasi warna keterlambatan
 * @returns {string} HTML string
 */
function generateHeaderRow(val, nomor, warnaTelatInfo) {
    let html = '';

    // Tentukan apakah row bisa di-highlight (untuk Jakarta)
    const rowHighlight = getRowHighlight(val);
    html += `<tr ${rowHighlight}>`;

    // Kolom packing button
    html += generatePackingButton(val);
    ...
    html += `<font onclick="detailPenjualanProduksiCabang('${val.penjualan_id}','${val.penjualan_detail_performa_id}')"><b>${nomorPenjualan}</b></font></td>`;
...
/**
 * Generate HTML untuk continuation row (item lanjutan dari nota yang sama)
 * @param {Object} val - Data item
 * @param {number} nomor - Nomor urut
 * @param {Object} warnaTelatInfo - Informasi warna keterlambatan
 * @returns {string} HTML string
 */
function generateContinuationRow(val, nomor, warnaTelatInfo) {
    let html = '<tr>';

    // Kolom kosong untuk packing
    html += `<td align="center" style="border-bottom: solid 1px; border-top: solid 1px; border-right: solid 1px; border-color:gray; padding:2px;" class="label-cell"></td>`;
...
/**
 * Mendapatkan atribut highlight untuk row (khusus Jakarta)
 * @param {Object} val - Data item
 * @returns {string} HTML attribute string
 */
function getRowHighlight(val) {
...
/**
 * Generate button packing dengan warna sesuai kondisi
 * @param {Object} val - Data item
 * @returns {string} HTML string untuk button packing
 */
function generatePackingButton(val) {
    let buttonHTML = '';
    ...
    return `<td style="border-bottom: solid gray 1px; border-top: solid 1px; border-right: solid 1px; border-color:gray; padding:2px;" class="label-cell" align="left">${buttonHTML}</td>`;
}

/**
 * Generate kolom-kolom data untuk row
 * @param {Object} val - Data item
 * @param {number} index - Index item
 ...
 * @returns {string} HTML string
 */
function generateDataColumns(val, index, backgroundColor, warnaTelatInfo, sisaKirimSJ, allData, now) {
    let html = '';

    // Kolom jenis penjualan dengan tombol status
    html += generateJenisColumn(val, index, warnaTelatInfo);

    // Kolom spesifikasi
    html += generateSpesifikasiColumn(val, backgroundColor);

    // Kolom keterangan
    html += generateKeteranganColumn(val, backgroundColor);

    // Kolom qty produksi
    html += `<td style="background:${backgroundColor}; border-bottom: solid 1px; border-left: solid 1px; border-color:gray;" class="label-cell" align="right">${parseInt(val.penjualan_qty) - parseInt(val.stok)}</td>`;

    // Kolom sisa
    html += generateSisaColumn(val, backgroundColor, sisaKirimSJ);

    // Kolom foto, SJC, dan tujuan kirim (hanya jika status selesai)
    if (val.status_produksi === 'selesai') {
        html += generateFotoButtons(val);
        html += generateSJCButton(val);
        html += generateTujuanKirimButton(val, sisaKirimSJ, allData);
    } else {
    ...
    // Kolom Invoice (BARU)
    html += generateInvoiceButton(val);

    return html;
}

/**
 * Generate kolom jenis penjualan dengan tombol-tombol status
 * @param {Object} val - Data item
 * @param {number} index - Index item
 * @param {Object} warnaTelatInfo - Informasi warna keterlambatan
 * @returns {string} HTML string
 */
function generateJenisColumn(val, index, warnaTelatInfo) {
...
/**
 * Generate kolom spesifikasi produk
 * @param {Object} val - Data item
 * @param {string} backgroundColor - Warna background
 * @returns {string} HTML string
 */
function generateSpesifikasiColumn(val, backgroundColor) {
    let spesifikasi = '';
...
/**
 * Generate kolom keterangan
 * @param {Object} val - Data item
 * @param {string} backgroundColor - Warna background
 * @returns {string} HTML string
 */
function generateKeteranganColumn(val, backgroundColor) {
    if (val.keterangan === null || val.keterangan === "") {
        return `<td style="background:${backgroundColor}; border-bottom: solid 1px; border-left: solid 1px; border-color:gray;" class="label-cell" align="center">-</td>`;
    }
    ...
    return `<td style="background:${backgroundColor}; border-bottom: solid 1px; border-left: solid 1px; border-color:gray;" class="label-cell" align="left">
		<p style="color:white; margin:0px;" class="text-add-colour-black-soft popup-open" data-popup=".detail-keterangan-cabang" onclick="keteranganCabang('${val.penjualan_detail_performa_id}')">${truncated}</p>
	</td>`;
}
 ...
 * @returns {string} HTML string
 */
function generateSisaColumn(val, backgroundColor, sisaKirimSJ) {
    let sisaValue = '';
    ...
    return `<td style="background:${backgroundColor}; border-left: solid gray 1px; border-bottom: solid gray 1px;" class="label-cell" align="center">${sisaValue}</td>`;
}

/**
 * Generate tombol foto produksi
 * @param {Object} val - Data item
 * @returns {string} HTML string
 */
function generateFotoButtons(val) {
 ...
 * @returns {string} HTML string
 */
function generateSJCButton(val) {
...
/**
 * Generate tombol tujuan kirim (Customer/Pusat)
 * @param {Object} val - Data item
 * @param {number} sisaKirimSJ - Sisa kirim surat jalan
 * @param {Object} allData - Semua data dari server
 * @returns {string} HTML string
 */
function generateTujuanKirimButton(val, sisaKirimSJ, allData) {
    // Tentukan warna button berdasarkan kondisi
    const colorBtnSJ = getColorButtonSJ(sisaKirimSJ, val.penjualan_total_kirim);
    const colorBtnTerima = getColorButtonTerima(val.total_terima_pabrik);
...
/**
 * Mendapatkan warna button surat jalan
 * @param {number} sisaKirimSJ - Sisa kirim surat jalan
 * @param {*} penjualanTotalKirim - Total kirim penjualan
 * @returns {string} CSS class untuk button
 */
function getColorButtonSJ(sisaKirimSJ, penjualanTotalKirim) {
    if (sisaKirimSJ <= 0) {
...
/**
 * Mendapatkan warna button terima pabrik
 * @param {*} totalTerimaPabrik - Total terima pabrik
 * @returns {string} CSS class untuk button
 */
function getColorButtonTerima(totalTerimaPabrik) {
...
/**
 * Generate tombol Invoice
 * Hanya ditampilkan jika lokasi_pabrik = 'Jakarta'
 * @param {Object} val - Data item
 * @returns {string} HTML string
 */
function generateInvoiceButton(val) {
    const lokasiPabrik = localStorage.getItem('lokasi_pabrik');
    const isJakarta = lokasiPabrik === 'Jakarta';

    // Tambahkan class invoice-cell untuk kontrol visibility
    let html = '<td class="label-cell invoice-cell popup-open" style="border-right: solid 1px; border-bottom: solid 1px; border-top: solid 1px; border-left: solid 1px; border-color:gray;';
    ...
    // Cek status invoice (0/false = belum ada, 1/true = sudah ada)
    if (val.status_invoice == 0 || val.status_invoice == false || val.status_invoice == null) {
        ...
        // Sudah ada invoice - button hijau untuk lihat detail
        html += ' data-popup=".popup-detail-invoice">';
        html += `<button class="btn-color-blueWhite button-small col button text-bold" style="width: 85px;" onclick="viewInvoiceDetail('${val.penjualan_id}','${val.penjualan_detail_performa_id}')">Invoice</button>`;
    }
...
/**
 * Generate HTML untuk tabel kosong
 * @returns {string} HTML string
 */
function generateEmptyTableHTML() {
    return `
		<tr>
			<td colspan="8">
...
/**
 * Generate HTML untuk footer tabel dengan total
 * @param {number} totalQty - Total quantity produksi
 * @returns {string} HTML string
 */
function generateFooterHTML(totalQty) {
    return `
		<tr>
			<td style="border-top:1px solid gray" colspan="8"></td>
